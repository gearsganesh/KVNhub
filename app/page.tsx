import { ArrowUpRight, Building2, Leaf, Lightbulb, Mail, MapPin, TrendingUp, Users } from "lucide-react";
import PortfolioStage, { WebsitePreview } from "../components/PortfolioStage";
import { businesses, capabilities, milestones, values } from "../data/site";

const valueIcons = [Users, Leaf, Lightbulb, TrendingUp];

export default function Home() {
  return <main>
    <header className="header"><div className="shell header-inner">
      <a href="#top" className="brand"><span className="brand-mark">KVN</span><span className="brand-sub">GROUP</span></a>
    ...
    </div><div className="capability-strip">{capabilities.map(([title,text],i)=><div className="capability" key={title}><span>{String(i+1).padStart(2,"0")}</span><h3>{title}</h3><p>{text}</p></div>)}</div></section>
    ...
    <section className="contact shell" id="contact"><div className="contact-card"><div><div className="section-label">CONNECT WITH KVN</div><h2>Let’s build<br/><em>what people value.</em></h2></div><div className="contact-info"><p>For business partnerships, destination opportunities, investments and group-level enquiries, connect with KVN Group.</p><a className="email" href="mailto:info@kvnhospitality.com">info@kvnhospitality.com <ArrowUpRight size={18}/></a><div className="meta"><span><MapPin size={15}/> Chennai, Tamil Nadu, India</span><span><Building2 size={15}/> Entertainment · Hospitality · Experiences</span><span><Mail size={15}/> Business enquiries</span></div></div></div></section>

    <footer><div className="shell footer-inner"><div className="footer-brand"><span className="brand-mark">KVN</span><span className="brand-sub">GROUP</span></div><p>© {new Date().getFullYear()} KVN Group. Building a Brighter Tomorrow.</p><div><a href="#businesses">Businesses</a><a href="#values">Values</a><a href="#contact">Contact</a></div></div></footer>
  </main>;
}
